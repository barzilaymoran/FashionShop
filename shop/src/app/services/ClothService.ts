...
export class ClothService {
  ...
  handleEachClothes(cloth) {
    var provider = cloth.provider;
    var clothObj = new Cloth(cloth._id, provider, cloth.description, cloth.imagePath, cloth.price, cloth.color, cloth.numberOfViews);
    ...
    this.http.post("http://localhost:3000/catalog/storeCloth", cloth, {headers: headers})
      .subscribe(
        function (res) {
          var clothId = res["id"];
          //.toLowerCase()
          var clothObj = new Cloth(clothId, cloth.provider, cloth.description, cloth.imagePath, cloth.price, cloth.color, cloth.numberOfViews);//addotional
    ...
    this.http.post("http://localhost:3000/catalog/updateCloth", data, {headers: headers})
      .subscribe(
        function(res) {
          ...
          //.toLowerCase()
          var clothObj = new Cloth(clothId, cloth.provider, cloth.description, cloth.imagePath, cloth.price, cloth.color, cloth.numberOfViews);//additional
  ...
  deleteCloth(id, cb) {
    var headers = this.userService.getHeaders();
    var self = this;
    this.http.delete("http://localhost:3000/catalog/deleteCloth/" + id, {headers: headers}).subscribe(
      function (res) {
  ...
  //additional
    updateClothViews(cloth, cb) {
    console.log("in update in client!!! " + cloth.id);
      var headers = this.userService.getHeaders();
      var self = this;
      this.http.post("http://localhost:3000/catalog/updateClothViews", cloth, {headers: headers}).subscribe(
        function(res) {
         // self.updateAll(id, null);
          cb({status: "success"});
        },
